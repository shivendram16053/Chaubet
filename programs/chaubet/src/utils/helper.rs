...
impl LMSR {
    ...
    pub fn cost_calculation(&self, yes_shares: &Decimal, no_shares: &Decimal) -> Result<Decimal> {
        ...
        let outcome_yes = div!(yes_shares, self.lmsr_b).exp(); // e.pow(q1/b)
        let outcome_no = div!(no_shares, self.lmsr_b).exp(); // e.pow(q2/b)
        let outcome_sum = add_or_sub!(outcome_yes, outcome_no, true)?; // e.pow(q1/b) + e.pow(q2/b)

        let cost = mul!(outcome_sum.ln(), self.lmsr_b); // b.ln(e.pow(q1/b) + e.pow(q2/b))
    ...
    pub fn share_calculation(
        ...
        let new_yes = add_or_sub!(self.outcome_yes_shares, Decimal::from(yes_shares), is_buy)?;
        let new_no = add_or_sub!(self.outcome_no_shares, Decimal::from(no_shares), is_buy)?;
